...
export default function MainHeader() {
  ...
  // اجلب نص شريط الإعلان العام (وهمي)
  useEffect(() => {
    // محاكاة البانر - معطل افتراضياً
    setBanner({ text: '', enabled: false });
  }, []);
        ...
        <div className="flex items-center gap-2" dir="rtl">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {open && (
              <div
                role="menu"
                className={`absolute ${activeLocale === 'ar' ? 'right-0' : 'left-0'} top-full mt-2 w-56 menu text-text-primary z-[1000]`}
              >
                <div className="py-1">
                  <button
                    role="menuitem"
                    className={`w-full ${activeLocale === 'ar' ? 'text-right' : 'text-left'} px-4 py-2 text-sm hover:bg-bg-surface-alt`}
                  ...
                  <button
                    role="menuitem"
                    className={`w-full ${activeLocale === 'ar' ? 'text-right' : 'text-left'} px-4 py-2 text-sm hover:bg-bg-surface-alt text-danger`}
                    onClick={() => {
                      setOpen(false);
