...
> ⚠️ **تنبيه:** قسم ZNET في هذا الملف **ملخّص قديم وفيه أخطاء** — أهمّها وصف
> `kupur` بأنه «اختياري» في `pin_ekle`، وهو **إلزامي** فعلياً (بدونه يردّ ZNET
> `Kupur Bilgisi Bulunamadı`). **المرجع المعتمد لـ ZNET هو الوثيقة الرسمية:**
> [`ZNET_API.md`](ZNET_API.md). عند أي تعارض، تلك هي الحاكمة.
